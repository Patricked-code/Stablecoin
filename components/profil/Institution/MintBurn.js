...
const MintBurn = () => {
    ...
    const [isLoggingInMintFrom, setIsLoggingInMintFrom] = useState(false);
    const [isLoggingInBurn, setIsLoggingInBurn] = useState(false);
    const [isLoggingInBurnFrom, setIsLoggingInBurnFrom] = useState(false);
    ...
    const [amountMintFrom, setAmountMintFrom] = useState();
    const [amountBurnFrom, setAmountBurnFrom] = useState();

    const [addressMintFrom, setAddressMintFrom] = useState();
    const [addressBurnFrom, setAddressBurnFrom] = useState();
    ...
    async function mintTokens() {
        ...
        try {
            ...
            if (window.ethereum && window.ethereum.isConnected()) {
                ...
                // const transaction = await contract.mintFrom("0x4bcc9911709eef442d44697c00cce7d5ea1f455f",amount);
                ...
                if (error.message.search('revert') >= 0) {
                    ...
                    Swal.fire({
                        ...
                        icon: 'error',
                        title: `Erreur`,
                        html:`<p> Une erreur de revert s'est produite. <br/>Assurez vous d'avoir le droit d'exécution de cette fonction.</p>`,
                        showConfirmButton: false,
                        timer: 20000
                    })
    ...
    // *************************************************************************************
        // FONCTION POUR EFFECTUER LE Burn
    // *************************************************************************************
    ...
    // *************************************************************************************
        // FONCTION POUR EFFECTUER LE MINTFrom
    // *************************************************************************************
    
    
    async function mintFromTokens() {
        setIsLoggingInMintFrom(true)
        ...
        try {
            ...
            if (window.ethereum && window.ethereum.isConnected()) {
                await window.ethereum.request({ method: "eth_requestAccounts" });
                const amount = ethers.utils.parseUnits(amountMintFrom, 10);
                // const transaction = await contract.mintFrom("0x4bcc9911709eef442d44697c00cce7d5ea1f455f",amount);
                const transaction = await contract.mintFrom(addressMintFrom,amount);
                
                await transaction.wait();
                if (transaction.hash) {
                    setIsLoggingInMintFrom(false)
                    Swal.fire({
                        position: 'top-center',
                        icon: 'success',
                        title: `Succès`,
                        html:`<p> Le <b>MintFrom</b> s'est effectué avec succès. Ci-dessous le hash du mint : <br/> ${transaction.hash} </p>`,
                        showConfirmButton: false,
                        timer: 20000
                    })
    ...
    // *************************************************************************************
        // FONCTION POUR EFFECTUER LE BurnFrom
    // *************************************************************************************
    
    
    async function burnFromTokens() {
        setIsLoggingInBurnFrom(true)
        try {
            // Exécutez votre appel de contrat ici
            ...
            if (window.ethereum && window.ethereum.isConnected()) {
                await window.ethereum.request({ method: "eth_requestAccounts" });
                const amount = ethers.utils.parseUnits(amountBurnFrom, 10);
                // const transaction = await contract.mint("0x88e036DCf477F016a605049e04952bb7BD828BD0",amount);
                const transaction = await contract.burnFrom(addressBurnFrom,amount);
                await transaction.wait();

                if (transaction.hash) {
                    setIsLoggingInBurnFrom(false)
                    Swal.fire({
                        position: 'top-center',
                        icon: 'success',
                        title: `Succès`,
                        html:`<p> Le <b>BurnFrom</b> s'est effectué avec succès. Ci-dessous le hash du burn : <br/> ${transaction.hash} </p>`,
                        showConfirmButton: false,
                        timer: 20000
                    })
                            ...
                            <div className="m-4 credit-card w-full lg:w-3/4 sm:w-auto shadow-lg  rounded-xl bg-white">
                                <div className='cryptocurrency-slides'>
                                    <div className='single-cryptocurrency-box'>
                                        <div className='align-items-center'>
                                            <div className='title'>
                                                <h3 className='text-center'>MintFrom </h3>
                                            </div>
                                            <hr/>
                                        </div>
                                        ...
                                        {/* <form> */}
                                            <div className='input-group-alternative m-3'>
                                                ...
                                                placeholder="Montant"
                                                className="form-control"
                                                defaultValue={amountMintFrom} 
                                                onChange={(event)=>setAmountMintFrom(event.target.value)}
                                            ...
                                            <div className='input-group-alternative m-3'>
                                                <label className="mx-2  mb-2" htmlFor='address'>
                                                    Adresse blockchain du recepteur
                                                </label>
                                                ...
                                                className="form-control"
                                                defaultValue={addressMintFrom} 
                                                onChange={(event)=>setAddressMintFrom(event.target.value)}
                                            ...
                                            {/* {currentAddressMetamask ? ( */}
                                                <div className='btn-box'>
                                                    <Button
                                                        ...
                                                        color="primary"
                                                        type="button"
                                                        onClick={mintFromTokens}
                                                        disabled={isLoggingInMintFrom}

                                                    >
                                                        Exécuter 
                                                        {isLoggingInMintFrom === true ? (<i className="fas fa-spinner fa-spin fa-lg"></i>) : ("")}
                                                    </Button>
                                                </div>
                                            {/* ) : ("")} */}
                                        </form>
                        ...
                        <div className='currency-selection'>
                            ...
                            <div className='cryptocurrency-slides'>
                                    <div className='single-cryptocurrency-box'>
                                        <div className=' align-items-center'>
                                            <div className='title '>
                                                <h3 className='text-center'>BurnFrom</h3>
                                            </div>
                                            <hr/>
                                        </div>
                                        <form onSubmit={handleSubmit}>
                                                <div className='input-group-alternative m-3'>
                                                    ...
                                                    placeholder="Montant"
                                                    className="form-control"
                                                    defaultValue={amountBurnFrom} 
                                                    onChange={(event)=>setAmountBurnFrom(event.target.value)}
                                                ...
                                                <div className='input-group-alternative m-3'>
                                                    ...
                                                    required='required'
                                                    id='address'
                                                    placeholder="Adresse blockchain du client"
                                                    className="form-control"
                                                    defaultValue={addressBurnFrom} 
                                                    onChange={(event)=>setAddressBurnFrom(event.target.value)}
                                                ...
                                                {/* {currentAddressMetamask ? ( */}
                                                    <div className='btn-box'>
                                                        <Button
                                                            ...
                                                            color="primary"
                                                            type="button"
                                                            onClick={burnFromTokens}
                                                            disabled={isLoggingInBurnFrom}
                                                        >
                                                            Exécuter
                                                            {isLoggingInBurnFrom === true ? (<i className="fas fa-spinner fa-spin fa-lg"></i>) : ("")}
                                                        </Button>
                                                    </div>
                                                {/* ) : ("")} */}
                                            </form>
