...
/**
 * Composant pour la troisième partie du quiz MIFID.
 * @component
 * @return {JSX.Element} Composant de la troisième partie du quiz MIFID.
 */
const CPartieThree = () => {
  ...
  // States de MIFID
  const [quizPartThree, setQuizPartThree] = useState([]);
  const [selectedOptions, setSelectedOptions] = useState([]);

  // Effet pour récupérer les questionnaires de la partie 3
  useEffect(async () => {
    const token = localStorage.getItem('tokenEnCours');
    const getQuizPartThree = async () => {
      try {
        const result = await fetch(`${API_URL}/api/mifid/find-quiz-part-three`, {
          headers: {
        ...
        setQuizPartThree(data.questions);
  ...
  const handleOptionChange = (event, questionIndex) => {
    ...
    const selectedOption = quizPartThree[questionIndex].reponses.find(
  ...
  // Fonction pour gérer la mise à jour des données de la partie 3
  const updatePartThree = async (event) => {
    event.preventDefault();
    setIsLoggingIn(true);

    try {
      const dataToUpdate = {
        pointOnePartThree: selectedOptions[0].points,
        pointTwoPartThree: selectedOptions[1].points,
        pointThreePartThree: selectedOptions[2].points,
        pointFourPartThree: selectedOptions[3].points,
      };

      const token = localStorage.getItem('tokenEnCours');
      const result = await fetch(`${API_URL}/api/mifid/update-part-three`, {
        method: 'PUT',
      ...
      if (data.message) {
        ...
        setTimeout(() => {
          Router.push('/profil/kyc/opcvm/mifid/partie-four');
        }, 5000);
      }
    } catch (error) {
        ...
        <div className='row mt-5'>
          ...
          <div className='m-4 credit-card w-full lg:w-3/4 sm:w-auto shadow-lg rounded-xl bg-white cryptocurrency-search-box login-form col-lg-6 col-md-12'>
            <form onSubmit={updatePartThree}>
              ...
              {quizPartThree.map((questionData, index) => (
                <div key={index}>
