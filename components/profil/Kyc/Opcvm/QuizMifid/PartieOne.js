...
const CPartieOne = () => {
  ...
  const [quizMifidOfUser, setQuizMifidOfUser] = useState('');

  // States de MIFID
  const [quizPartOne, setQuizPartOne] = useState([]);
  const [selectedOptions, setSelectedOptions] = useState([]);

  // Effet pour récupérer les questionnaires de la partie 1
  useEffect(async () => {
    const token = localStorage.getItem('tokenEnCours');
    const getQuizPartOne = async () => {
      try {
  ...
  const addPartOne = async (event) => {
    ...
    try {
      ...
      const token = localStorage.getItem('tokenEnCours');
      const result = await fetch(`${API_URL}/api/mifid/add-part-one`, {
        method: 'POST',
      ...
      if (data.message) {
        ...
        setTimeout(() => {
          Router.push('/profil/kyc/opcvm/mifid/partie-two');
        }, 5000);
      }
    } catch (error) {
  ...
  const updatePartOne = async (event) => {
    event.preventDefault();
    setIsLoggingIn(true);
    ...
    try {
      ...
      const token = localStorage.getItem('tokenEnCours');
      const result = await fetch(`${API_URL}/api/mifid/update-part-one`, {
        method: 'PUT',
      ...
      <div className='mt-15' >
        ...
        <div className='row mt-5'>
          ...
          <div className='m-4 credit-card w-full lg:w-3/4 sm:w-auto shadow-lg rounded-xl bg-white cryptocurrency-search-box login-form col-lg-6 col-md-12'>
            <form onSubmit={quizMifidOfUser?.userId || !quizMifidOfUser?.userId === undefined ? updatePartOne : addPartOne}>
