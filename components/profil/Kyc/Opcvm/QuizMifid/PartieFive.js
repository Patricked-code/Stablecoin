...
/**
 * Composant pour la cinquième partie du quiz MIFID.
 * @component
 * @return {JSX.Element} Composant de la cinquième partie du quiz MIFID.
 */
const CPartieFive = () => {
  ...
  // States de MIFID
  const [quizPartFive, setQuizPartFive] = useState([]);
  const [selectedOptions, setSelectedOptions] = useState([]);

  // Effet pour récupérer les questionnaires de la partie 5
  useEffect(async () => {
    const token = localStorage.getItem('tokenEnCours');
    const getQuizPartFive = async () => {
      try {
        const result = await fetch(`${API_URL}/api/mifid/find-quiz-part-five`, {
          headers: {
        ...
        setQuizPartFive(data.questions);
  ...
  const handleOptionChange = (event, questionIndex) => {
    ...
    const selectedOption = quizPartFive[questionIndex].reponses.find(
  ...
  // Fonction pour gérer la mise à jour des données de la partie 5
  const updatePartFive = async (event) => {
    event.preventDefault();
    setIsLoggingIn(true);

    try {
      const dataToUpdate = {
        pointOnePartFive: selectedOptions[0].points,
        pointTwoPartFive: selectedOptions[1].points,
        pointThreePartFive: selectedOptions[2].points,
        pointFourPartFive: selectedOptions[3].points,
      };

      const token = localStorage.getItem('tokenEnCours');
      const result = await fetch(`${API_URL}/api/mifid/update-part-five`, {
        method: 'PUT',
      ...
      if (data.message) {
        ...
        setTimeout(() => {
          Router.push('/profil/kyc/opcvm/mifid/partie-six');
        }, 5000);
      }
    } catch (error) {
        ...
        <div className='row mt-5'>
          ...
          <div className='m-4 credit-card w-full lg:w-3/4 sm:w-auto shadow-lg rounded-xl bg-white cryptocurrency-search-box login-form col-lg-6 col-md-12'>
            <form onSubmit={updatePartFive}>
              ...
              {quizPartFive.map((questionData, index) => (
                <div key={index}>
