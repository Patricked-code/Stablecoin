...
/**
 * Composant pour la sixième partie du quiz MIFID.
 * @component
 * @return {JSX.Element} Composant de la sixième partie du quiz MIFID.
 */
const CPartieSix = () => {
  ...
  // States de MIFID
  const [quizPartSix, setQuizPartSix] = useState([]);
  const [selectedOptions, setSelectedOptions] = useState([]);

  // Effet pour récupérer les questionnaires de la partie 6
  useEffect(async () => {
    const token = localStorage.getItem('tokenEnCours');
    const getQuizPartSix = async () => {
      try {
        const result = await fetch(`${API_URL}/api/mifid/find-quiz-part-six`, {
          headers: {
  ...
  const handleOptionChange = (event, questionIndex) => {
    ...
    const selectedOption = quizPartSix[questionIndex].reponses.find(
  ...
  // Fonction pour gérer la mise à jour des données de la partie 6
  const updatePartSix = async (event) => {
    event.preventDefault();
    setIsLoggingIn(true);

    try {
      const dataToUpdate = {
        pointOnePartSix: selectedOptions[0].points,
        pointTwoPartSix: selectedOptions[1].points,
        pointThreePartSix: selectedOptions[2].points,
        pointFourPartSix: selectedOptions[3].points,
      };

      const token = localStorage.getItem('tokenEnCours');
      const result = await fetch(`${API_URL}/api/mifid/update-part-six`, {
        method: 'PUT',
      ...
      if (data.message) {
        ...
        setTimeout(() => {
          Router.push('/profil/kyc/opcvm/mifid/partie-seven');
        }, 5000);
      }
    } catch (error) {
        ...
        <div className='row mt-5'>
          ...
          <div className='m-4 credit-card w-full lg:w-3/4 sm:w-auto shadow-lg rounded-xl bg-white cryptocurrency-search-box login-form col-lg-6 col-md-12'>
            <form onSubmit={updatePartSix}>
              ...
              {quizPartSix.map((questionData, index) => (
                <div key={index}>
