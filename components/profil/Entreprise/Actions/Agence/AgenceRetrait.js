...
const AgenceRetrait = () => {
  ...
  // const position = [parseFloat(oneKycForParticular?.latitude), parseFloat(oneKycForParticular?.longiitude)];
    ...
    const [agenceCommission, setAgenceCommission] = useState(0);
    ...
    //  const montantRetirerAvecFrais =  parseFloat(montantRetirer) + frais 
     // Fin
      ...
      useEffect(() => {
        ...
        const frais = montantRetirer*dataRequestUseStablecoin?.percentageWithdrawal/100
        const montantRetirerAvecFrais =  parseFloat(montantRetirer) + frais 
        setMontantRetirerAvecFrais(montantRetirerAvecFrais)
        const commissionAgency = frais*dataRequestUseStablecoin?.percentageWithdrawalAgency/100
        const commissionWti = frais*dataRequestUseStablecoin?.percentageWithdrawalWealthtech/100
        
       
        // setFees(frais)
        // setAmount(montantEnvoyer)
        setWealthtechCommission(commissionWti)
        setAgenceCommission(commissionAgency)
        setFees(frais)
      };
        getInfos()
      },[montantRetirer])
...
const dumpVariables = () =>{
  ...
  const addWithdrawalCash = async (_amountBurn, _wealthtechCommission, _agenceCommission, _amount, _addressCustomer, _addressInstitution, _hash) => {
    setIsLoggingIn(true);
    let nameInstitution =""
    ...
    try {
        ...
        const dataBody = {
          ...
          fees: fees,
          agenceCommission: _agenceCommission,
  ...
  const transferBatchWithBurn = async () => {
    ...
    try {
      ...
      const tostingC = String(agenceCommission);
      ...
      const dataForm = {
        ...
        recipients: [dataRequestUseStablecoin?.wealthtechCommissionAddress, magicCurrentAddress],
  ...
  // FONCTION POUR INITIER LE RETRAIT
  const initiateWithdrawal = async () => {
    setIsLoggingIn(true);

    let customerName =""
    if (infosOtherUser?.codeTypeProfil=="part") {
        customerName = infosOtherUser?.lastName + ' ' + infosOtherUser?.firstName
    } else {
        customerName = infosOtherUser?.entreprise
    }

    try {
        
        const dataBody = {
            typeTransaction: "Retrait cash agence",
            nameCustomer: customerName,
            nameAgency: dataRequestUseStablecoin?.shopName,
            addressCustomer: infosOtherUser?.address,
            addressAgency: dataRequestUseStablecoin?.agencyCommissionAddress,
            amount: montantRetirer,
            fees: fees,
            agencyCommission: agenceCommission,
            wealthtechCommission: wealthtechCommission,
            customerId: infosOtherUser?.id,
        }
        ...
        const response = await fetch(`${API_URL}/api/transaction/add-agency-cash-withdrawal`, {
            method: 'POST',
            body: JSON.stringify(dataBody),
            headers: {
                'Content-Type': 'application/json',
        ...
        */
        if (data.message==200) {
          ...
          Swal.fire({
            position: 'center',
            icon: 'success',
            html: `<p> La demande du retrait est envoyée avec succès.</p>`,
            showConfirmButton: false,
            timer: 5000
          });
    ...
    <>
                                  ...
                                  <>
                                    ...
                                    <div className=" row col-lg-12 col-md-12 justify-content-between">
                                      ...
                                      {/* <div className='col-lg-6 col-md-6 text-center my-5'>
                                        ...
                                        {oneKycForParticular?.latitude && oneKycForParticular?.longitude?(
        ...
        {/* ********************************************************************************** */}
            ...
            <Modal show={showWithdrawal} className="mt-15" onHide={handleWithdrawalClose} style={{maxWidth: '1800px', width: '100%'}}>
                ...
                <Modal.Title className="" >Retrait cash à l'agence</Modal.Title>
                </Modal.Header>
                {/* <form > */}
                <Modal.Body>
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="form-group my-6 ">
                          ...
                          <Row className="my-3 justify-content-between align-items-center">
                            ...
                            >
                              <Button variant="success" onClick={initiateWithdrawal}  disabled={isLoggingIn} className="text-white" >
                                Envoyer
