...
const CarteAgences = () => {
    ...
    useEffect(() => {
        const fetchCommerces = async () => {
            try {
                const response = await fetch(`${API_URL}/api/payment-request/${selectedTrade === '' ? 'find-all-request-use-stablecoin-active-agency' : 'searche-request-use-stablecoin-agency?searchElement=' + encodeURIComponent(selectedTrade)}`, {
                    method: 'GET',
                    headers: {
        ...
        <div style={{ display: 'flex', flexDirection: 'row', gap: '20px' }}>
            ...
            <div style={{ width: '30%', padding: '20px', overflowY: 'auto', maxHeight: '800px', borderRight: '1px solid #ccc' }}>
                <select value={selectedTrade} onChange={(e) => setSelectedTrade(e.target.value)} style={{ width: '100%', padding: '10px', marginBottom: '20px' }}>
                    <option value="">Sélectionnez un type de services</option>
                ...
                {visibleCommerces.map((commerce, index) => (
                    <div key={commerce.id} style={{ marginBottom: '20px' }}>
                        ...
                        <p ><i className='colorBlue py-0'>{formatProductTypes(commerce?.serviceType)} </i><br/>
