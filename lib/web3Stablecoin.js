...
export async function ensureMoonbaseAlphaNetwork() {
  ...
  if (!ethereum) {
    throw new Error('Aucun wallet Web3 détecté. Installez MetaMask ou un wallet compatible EIP-1193.');
  }
...
export function getInjectedProvider() {
  ...
  if (!ethereum) {
    throw new Error('Aucun wallet Web3 détecté.');
  }
...
export async function readStablecoinInfoForAddress(walletAddress) {
  ...
  return {
    contractAddress: ethers.utils.getAddress(getStablecoinAddress()),
